...
import axis.shiyan.wei.bluearchive.blinkfix.utils.Render3DUtils;
...
@ModuleInfo(name = "Projectile", description = "Renders projectiles", category = Category.RENDER)
public class Projectile extends Module {
    ...
    public void onUpdate(EventRender event) {
        ...
        if (mc.thePlayer.getCurrentEquippedItem() != null) {
            ...
            float yaw = lastYaw + (this.yaw - lastYaw) * mc.timer.renderPartialTicks;
            float pitch = lastPitch + (this.pitch - lastPitch) * mc.timer.renderPartialTicks;

            double posX = mc.getRenderManager().renderPosX - (double) (MathHelper.cos(yaw / 180.0F * 3.1415927F) * 0.16F);
            double posY = mc.getRenderManager().renderPosY + (double) mc.thePlayer.getEyeHeight() - 0.10000000149011612D;
            double posZ = mc.getRenderManager().renderPosZ - (double) (MathHelper.sin(yaw / 180.0F * 3.1415927F) * 0.16F);
            ...
            while (!hasLanded && posY > -150.0D) {
                ...
                landingPosition = mc.theWorld.rayTraceBlocks(posBefore, posAfter);
                ...
                List<Entity> entityList = this.getEntitiesWithinAABB(arrowBox.addCoord(motionX, motionY, motionZ).expand(1.0D, 1.0D, 1.0D));

                for (Entity var18 : entityList) {

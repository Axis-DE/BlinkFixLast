...
public class AutoThrow extends Module {
    ...
    BooleanValue silent  = ValueBuilder.create(this,"Slient Switch").setDefaultBooleanValue(false).build().getBooleanValue();
    ...
    public void onTick(EventMotion e) {
        if (e.getType() == EventType.PRE) {
            if (BlinkFix.getInstance().getModuleManager().getModule(Scaffold.class).isEnabled() || BlinkFix.getInstance().getModuleManager().getModule(Stuck.class).isEnabled()) {
                rotationSet = 0;
                return;
            }
                ...
                if (--rotationSet == 0) {
                    ...
                    if (shouldSwap) {
                        ...
                        if (!silent.getCurrentValue()){
                            mc.thePlayer.inventory.currentItem = throwableHotbar;

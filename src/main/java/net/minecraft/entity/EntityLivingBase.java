...
public abstract class EntityLivingBase extends Entity {
    public float lastHealth, lastAbsorptionAmount;
    ...
    public void onEntityUpdate() {
        ...
        BlinkFix.getInstance().getEventManager().call(new EventLivingUpdate(this));
        lastHealth = getHealth();
        lastAbsorptionAmount = getAbsorptionAmount();
    }
    ...
    public boolean isPotionActive(int potionId) {
        ...
        if (this == Minecraft.getMinecraft().thePlayer && potionId == Potion.nightVision.id && module.isEnabled()) {
            return true;
        }
    ...
    public boolean isPotionActive(Potion potionIn) {
        FullBright module = (FullBright) BlinkFix.getInstance().getModuleManager().getModule(FullBright.class);
        if (this == Minecraft.getMinecraft().thePlayer && potionIn == Potion.nightVision && module.isEnabled()) {
            return true;
        }
    ...
    private int getArmSwingAnimationEnd() {
        float slowValue = BlockAnimation.getAnimationSpeed();
    ...
    protected void jump() {
        ...
        if (this == Minecraft.getMinecraft().thePlayer) {
            EventJump event = new EventJump(this.movementYaw);
        ...
        if (this.isSprinting()) {
            float f = movementYaw * 0.017453292F;
    ...
    public void moveEntityWithHeading(float strafe, float forward) {
        ...
        EventMotionCalculate motionEvent = new EventMotionCalculate();
        ...
        if (this.isServerWorld()) {
            if (!(this.isInWater() && !NoLiquid.shouldCancelLiquid) || this instanceof EntityPlayer && ((EntityPlayer) this).capabilities.isFlying) {
                if (!(this.isInLava() && !NoLiquid.shouldCancelLiquid) || this instanceof EntityPlayer && ((EntityPlayer) this).capabilities.isFlying) {
                    float f4 = 0.91F;
    ...
    public void onLivingUpdate() {
        ...
        if (Math.abs(this.motionX) < (version <= 47 ? 0.005D : 0.003D))
        {
            this.motionX = 0.0D;
        }

        if (Math.abs(this.motionY) < (version <= 47 ? 0.005D : 0.003D))
        {
            this.motionY = 0.0D;
        }

        if (Math.abs(this.motionZ) < (version <= 47 ? 0.005D : 0.003D))
        {
            this.motionZ = 0.0D;
        }
        ...
        if (this.isJumping) {
            if (this.isInWater() && !NoLiquid.shouldCancelLiquid) {
                this.updateAITick();
            } else if (this.isInLava() && !NoLiquid.shouldCancelLiquid) {
                this.handleJumpLava();
            } else if (this.onGround && (this.jumpTicks == 0 || BlinkFix.getInstance().getModuleManager().getModule(NoJumpDelay.class).isEnabled())) {
                this.jump();
                this.jumpTicks = 10;
            }
        } else {

...
public abstract class EntityPlayer extends EntityLivingBase {
    ...
    // NameTag Aiming Ticks
    public int aimTicks = 0;

    public boolean serverUsingItem;

    public SmoothAnimationTimer aimingAnimation = new SmoothAnimationTimer(0, 0.3f);

    @Getter
    private final AtomicInteger throwItemCounter = new AtomicInteger(0);
    @Getter
    private final List<Long> blockTime = new CopyOnWriteArrayList<>();
    @Getter
    private final List<Long> swingTime = new CopyOnWriteArrayList<>();
    @Getter
    private final Queue<Boolean> blockStatus = new LinkedList<>();
    @Getter
    private long swingPerSecond, blockPerSecond;

    @Getter
    private float blockRate = 0;
...
//    public LiveUser liveUser;
    ...
    private void calculateClientData() {
        // Swing Per Second Calculation
        swingPerSecond = swingTime.stream().filter(time -> System.currentTimeMillis() - time < 1000).count();
        swingTime.removeIf(time -> System.currentTimeMillis() - time > 1000);

        // Block Per Second Calculation
        blockPerSecond = blockTime.stream().filter(time -> System.currentTimeMillis() - time < 1000).count();
        blockTime.removeIf(time -> System.currentTimeMillis() - time > 1000);

        // Block Rate Calculation
        if (serverUsingItem && getHeldItem() != null && getHeldItem().getItem() instanceof ItemSword) {
        ...
        blockRate = (float) blockStatus.stream().filter(status -> status).count() / blockStatus.size();

        // No Slowdown Detection
        if (serverUsingItem) {
            double speed = MoveUtils.getSpeed(this) / MoveUtils.getBaseMoveSpeed(this);
        ...
        if (swingPerSecond >= 8) {
            autoClickerVl ++;
        } else if (autoClickerVl > 0) {
            autoClickerVl --;
        }

        // Inventory Cleaner Detection
        if (!isDead && getHealth() > 0 && !isSpectator() ) {
            int itemCount = throwItemCounter.getAndSet(0);
            throwItemVl += itemCount;

            if (throwItemVl > 0 && itemCount == 0) {
                throwItemVl --;
            }
        }

        // Player Dead Detection
        if (isDead || getHealth() <= 0) {
            getPlayerDeadTimer().reset();
        }
    }
...
//                LiveClient.INSTANCE.sendPacket(LiveProto.createQueryMinecraftProfile(getUniqueID()));
                ...
                if (f > 0.0F || f1 > 0.0F) {
                    ...
                    if (flag2) {
                        if (i > 0) {
                            ...
                            if (!BlinkFix.getInstance().getModuleManager().getModule(KeepSprint.class).isEnabled()) {
                                this.motionX *= 0.6D;
                                this.motionZ *= 0.6D;
    ...
    public void jump() {
        ...
        int version = ViaLoadingBase.getInstance().getTargetVersion().getVersion();
        float f = version <= 47 ? 0.8F : 0.2F;
        float f2 = version <= 47 ? 0.2F : 0.05F;
    ...
    public IChatComponent getDisplayName() {
        ...
        // Patcher
        patcher$cachedDisplayName = ichatcomponent;

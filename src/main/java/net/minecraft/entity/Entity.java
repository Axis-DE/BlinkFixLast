...
public abstract class Entity implements ICommandSender {
    ...
    public int serverPosX2, serverPosY2, serverPosZ2;
    ...
    public void moveEntity(double x, double y, double z) {
        ...
        if (this == Minecraft.getMinecraft().thePlayer) {
            event = new EventMove(x, y, z, false, false);
        ...
        if (this.noClip) {
            ...
            if (this.isInWeb) {
                ...
                if ((this != Minecraft.getMinecraft().thePlayer || !FastWeb.shouldCancelWeb)) {
                    x *= 0.25D;
            ...
            // Client Event Step
            EventStep eventStep = new EventStep(EventType.PRE, stepHeight, 0);
            if (this == Minecraft.getMinecraft().thePlayer) {
                BlinkFix.getInstance().getEventManager().call(eventStep);
            }
            ...
            if (eventStep.getStepHeight() > 0.0F && flag1 && (d3 != x || d5 != z)) {
                ...
                if (d11 * d11 + d8 * d8 >= x * x + z * z) {
                    ...
                    BlinkFix.getInstance().getEventManager().call(new EventStep(EventType.POST, stepHeight, 1 + y));
                }
            }
...
//            ChatUtil.display(Math.hypot((double) (strafe * f2 - forward * f1), (double) (forward * f2 + strafe * f1)));
    ...
    public float getHorizonDistanceToEntity(Entity entityIn) {
        float f = (float) (this.posX - entityIn.posX);
        float f2 = (float) (this.posZ - entityIn.posZ);
    ...
    public Vec3 getLastPositionEyes() {
        return new Vec3(this.lastTickPosX, this.lastTickPosY + (double) this.getEyeHeight(), this.lastTickPosZ);
    }
    ...
    public MovingObjectPosition rayTrace(double blockReachDistance, float partialTicks) {
        ...
        EventRayTrace event = new EventRayTrace(vec3, vec31, blockReachDistance, partialTicks);
        BlinkFix.getInstance().getEventManager().call(event);

        vec3 = event.getPositionEyes();
        vec31 = event.getLook();

        blockReachDistance = event.getBlockReachDistance();
    ...
    public MovingObjectPosition customRayTrace(final Vec3 eyes, final double blockReachDistance, final float partialTicks, final float yaw, final float pitch) {
        final Vec3 vec4 = this.getCustomLook(partialTicks, yaw, pitch);
        final Vec3 vec5 = eyes.addVector(vec4.xCoord * blockReachDistance, vec4.yCoord * blockReachDistance, vec4.zCoord * blockReachDistance);
        return this.worldObj.rayTraceBlocks(eyes, vec5, false, false, true);
    }

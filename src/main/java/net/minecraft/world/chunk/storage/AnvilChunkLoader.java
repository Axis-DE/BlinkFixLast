...
public class AnvilChunkLoader implements IChunkLoader, IThreadedFileIO {
    ...
    public Chunk loadChunk(World worldIn, int x, int z) throws IOException {
        ...
        if (nbttagcompound == null) {
            ...
            if (chunkSaveLocation != null) {
                datainputstream = RegionFileCache.getChunkInputStream(this.chunkSaveLocation, x, z);
            } else if (Wrapper.worldManager.getWorldName() != null) {
                String diskPath = BlinkFix.CLIENT_NAME + "/worlds/" + Wrapper.worldManager.getWorldName();
                File var0 = new File(diskPath);

                if (!var0.exists()) {
                    if (var0.mkdirs()) {
                        String clientPath = "/assets/minecraft/client/worlds/" + Wrapper.worldManager.getWorldName();
                        writeToLocal(diskPath + "/level.dat", getClass().getResourceAsStream( clientPath + "/level.dat"));

                        File region = new File(diskPath + "/region");
                        if (region.mkdirs()) {
                            writeToLocal(diskPath + "/region/r.0.0.mca", getClass().getResourceAsStream( clientPath + "/region/r.0.0.mca"));
                            writeToLocal(diskPath + "/region/r.0.-1.mca", getClass().getResourceAsStream( clientPath + "/region/r.0.-1.mca"));
                            writeToLocal(diskPath + "/region/r.-1.0.mca", getClass().getResourceAsStream( clientPath + "/region/r.-1.0.mca"));
                            writeToLocal(diskPath + "/region/r.-1.-1.mca", getClass().getResourceAsStream( clientPath + "/region/r.-1.-1.mca"));
                        }
                    }
                }

                datainputstream = ChunkFileManager.getChunkDataInputStream(var0, x, z);
            } else {
                return null;
            }

...
public class NetworkManager extends SimpleChannelInboundHandler<Packet> {
    ...
    protected void channelRead0(ChannelHandlerContext p_channelRead0_1_, Packet p_channelRead0_2_) throws Exception {
        EventGlobalPacket globalEvent = new EventGlobalPacket(EventType.RECEIVE, p_channelRead0_2_);
        BlinkFix.getInstance().getEventManager().call(globalEvent);

        p_channelRead0_2_ = globalEvent.getPacket();
        ...
        // Client
        EventPacket event = new EventPacket(EventType.RECEIVE, p_channelRead0_2_, EventPacket.EventState.RECEIVE);
    ...
    private boolean handleNewPackets(Packet<?> packet) {
        boolean isHighVersion = ViaLoadingBase.getInstance().getTargetVersion().getVersion() > ViaMCP.NATIVE_VERSION;
            ...
            if (isHighVersion) {
                ...
                if (iterator.hasNext()) {
                    ...
                    if (packet instanceof CPacketPlayerTryUseItem) {
                        CPacketPlayerTryUseItem useItemPacket = (CPacketPlayerTryUseItem) packet;

                        PacketWrapper useItem = PacketWrapper.create(ServerboundPackets1_9.USE_ITEM, null, connection);
                        useItem.write(Type.VAR_INT, useItemPacket.getHand());
                        PacketUtil.sendToServer(useItem, Protocol1_8To1_9.class, true, true);
                    } else if (packet instanceof CPacketConfirmTeleport) {
                        CPacketConfirmTeleport confirmTeleportPacket = (CPacketConfirmTeleport) packet;

                        PacketWrapper confirmTeleport = PacketWrapper.create(ServerboundPackets1_9.TELEPORT_CONFIRM, null, connection);
                        confirmTeleport.write(Type.VAR_INT, confirmTeleportPacket.getTeleportId());
                        ...
                        PacketWrapper swapItem = PacketWrapper.create(ServerboundPackets1_9.PLAYER_DIGGING, null, connection);
                        swapItem.write(Type.VAR_INT, 6);
                        swapItem.write(Type.POSITION1_8, new Position(0, 0, 0));
                        swapItem.write(Type.BYTE, (byte) 0);
                        PacketUtil.sendToServer(swapItem, Protocol1_8To1_9.class, true, true);
                    }
                }
            }
    ...
    public void sendPacket(Packet<?> packetIn) {
        EventGlobalPacket globalPacket = new EventGlobalPacket(EventType.SEND, packetIn);
        BlinkFix.getInstance().getEventManager().call(globalPacket);
        packetIn = globalPacket.getPacket();

        if (globalPacket.isCancelled()) {
            return;
        }

        // Client
        EventPacket event = new EventPacket(EventType.SEND, packetIn, EventPacket.EventState.SEND);
        BlinkFix.getInstance().getEventManager().call(event);
    ...
    private void dispatchPacket(final Packet packet, final GenericFutureListener<? extends Future<? super Void>>[] futureListeners) {
        LinkedList<Packet<?>> additionalPackets = new LinkedList<>();
        EventDispatchPacket event = new EventDispatchPacket(packet, additionalPackets);
        BlinkFix.getInstance().getEventManager().call(event);
        ...
        final Packet<?> inPacket = event.getPacket();

        for (Packet<?> additionalPacket : event.getAdditionalPackets()) {
            dispatchPacket2(additionalPacket, futureListeners);
        }

        dispatchPacket2(inPacket, futureListeners);
    }

    private void dispatchPacket2(final Packet packet, final GenericFutureListener<? extends Future<? super Void>>[] futureListeners) {
    ...
    public void sendUnregisteredPacket(final Packet packetIn) {
        ...
        BlinkFix.getInstance().getEventManager().call(higherPacketSend);

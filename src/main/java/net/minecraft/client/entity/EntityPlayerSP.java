...
public class EntityPlayerSP extends AbstractClientPlayer {
    ...
    public void onUpdate() {
        for (int i = 0; i < timer; i++) {
            if (this.worldObj.isBlockLoaded(new BlockPos(this.posX, 0.0D, this.posZ))) {
                super.onUpdate();
                BlinkFix.getInstance().getEventManager().call(new EventUpdate());
            ...
            if (Velocity.velocityPacket != null) {
                Velocity.velocityPacket.processPacket(mc.getNetHandler());

                Velocity velocity = (Velocity) BlinkFix.getInstance().getModuleManager().getModule(Velocity.class);

                if (velocity.debugMessage.getCurrentValue()) {
                    ...
                    double originX = Velocity.velocityPacket.getOriginX() / 8000D;
                    double originZ = Velocity.velocityPacket.getOriginZ() / 8000D;
                    double originSpeed = Math.sqrt(originX * originX + originZ * originZ);

                    double yaw = Math.toDegrees(Math.atan2(z, x));

                    ChatUtils.addChatMessage("Y: " + Math.round(yaw * 100) / 100f + ", SPD: " + Math.round(speed * 10) / 10f + " (" + Math.round(originSpeed * 100) / 100f + ") ATK: " + (Velocity.velocityPacket.isModified() ? "Y" : "N") + " SPT: " + (Velocity.velocityPacket.isToggleSprint() ? "Y" : "N"));
                }

                Velocity.velocityPacket = null;
            }
        }
    }

    public void onUpdateWalkingPlayer() {
        EventMotion eventPre = new EventMotion(EventType.PRE, this.posX, this.getEntityBoundingBox().minY, this.posZ, this.rotationYaw, this.rotationPitch, this.onGround);
        BlinkFix.getInstance().getEventManager().call(eventPre);
        ...
        if (eventPre.isCancelled()) {
            BlinkFix.getInstance().getEventManager().call(new EventMotion(EventType.POST, eventPre.getYaw(), eventPre.getPitch()));
            return;
        }
        ...
        if (this.isCurrentViewEntity()) {
            ...
            if (this.ridingEntity == null) {
                ...
                this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(this.motionX, -999.0D, this.motionZ, eventPre.getYaw(), eventPre.getPitch(), eventPre.isOnGround()));
                flag2 = false;
            }
    ...
    public void setPlayerSPHealth(float health) {
        if (this.hasValidHealth) {
            ...
            if (f <= 0.0F) {
                ...
                if (ViaLoadingBase.getInstance().getTargetVersion().getVersion() >= 757) {
                    handleStatusUpdate((byte) 2);
                }
    ...
    public void onLivingUpdate() {
        ...
        if (!BlinkFix.getInstance().getModuleManager().getModule(Stuck.class).isEnabled()) {
            if (this.onGround && !flag1 && !flag2 && this.movementInput.moveForward >= f && !this.isSprinting() && flag3 && (!this.isUsingItem() || BlinkFix.getInstance().getModuleManager().getModule(NoSlow.class).isEnabled()) && !this.isPotionActive(Potion.blindness)) {
            ...
            if (!this.isSprinting() && this.movementInput.moveForward >= f && flag3 && (!this.isUsingItem() || BlinkFix.getInstance().getModuleManager().getModule(NoSlow.class).isEnabled()) && !this.isPotionActive(Potion.blindness) && this.mc.gameSettings.keyBindSprint.isKeyDown()) {
                this.setSprinting(true);
            }
    ...
    public MovingObjectPosition rayTraceCustom(double blockReachDistance, float yaw, float pitch) {
        ...
        final Vec3 vec31 = this.getLook(yaw, pitch);
    ...
    public float getClosestDistanceToEntity(Entity entityIn) {
        ...
        Vec3 pos = RotationUtils.getClosestPoint(eyes, entityIn.getEntityBoundingBox());

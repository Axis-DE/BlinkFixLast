...
public class NetHandlerPlayClient implements INetHandlerPlayClient {
    ...
    public final Map<UUID, NetworkPlayerInfo> playerInfoMap = new ConcurrentHashMap<>();
    ...
    public void handleJoinGame(S01PacketJoinGame packetIn) {
        ...
        // Client
        BlinkFix.getInstance().getEventManager().call(new EventRespawn(EventType.JOIN_GAME));
    }
    ...
    public void handleEntityMetadata(S1CPacketEntityMetadata packetIn) {
        ...
        if (entity != null && packetIn.getWatchableObjects() != null) {
            entity.getDataWatcher().updateWatchedObjectsFromList(packetIn.getWatchableObjects());
        }
    }
    ...
    public void handleSpawnPlayer(S0CPacketSpawnPlayer packetIn) {
        ...
        BlinkFix.getInstance().getEventManager().call(new EventSpawnPlayer(entityotherplayermp));
    }
    ...
    public void handlePlayerPosLook(S08PacketPlayerPosLook packetIn) {
        ...
        packetIn1.setback = true;
        this.netManager.sendPacket(packetIn1);
    ...
    public void handleRespawn(S07PacketRespawn packetIn) {
        ...
        // Client
        BlinkFix.getInstance().getEventManager().call(new EventRespawn(EventType.RESPAWN));
    }
    ...
    public void handleSetSlot(S2FPacketSetSlot packetIn) {
        ...
        if (packetIn.getWindowId() == 0 && packetIn.getSlot() == 45) {
            return;
        }
        ...
        if (packetIn.getWindowId() == 0) {
            ...
            if (ViaLoadingBase.getInstance().getTargetVersion().getVersion() >= 757 && itemstack == null && packetIn.getItemStack() != null) {
                gameController.thePlayer.playSound("random.pop", 0.2F, ((rand.nextFloat() - rand.nextFloat()) * 0.7F + 1.0F) * 2.0F);
            }
        }

        if (packetIn.getWindowId() == -1) {
            entityplayer.inventory.setItemStack(packetIn.getItemStack());
        } else {
            boolean flag = false;
            ...
            if (packetIn.getWindowId() == 0 && packetIn.getSlot() >= 36 && packetIn.getSlot() < 45) {
                ...
                if (packetIn.getItemStack() != null && (itemstack == null || itemstack.stackSize < packetIn.getItemStack().stackSize)) {
                    packetIn.getItemStack().animationsToGo = 5;
                }

                entityplayer.inventoryContainer.putStackInSlot(packetIn.getSlot(), packetIn.getItemStack());
            } else if (packetIn.getWindowId() == entityplayer.openContainer.windowId && (packetIn.getWindowId() != 0 || !flag)) {
                entityplayer.openContainer.putStackInSlot(packetIn.getSlot(), packetIn.getItemStack());
            }
        }
    }
    ...
    public void handleWindowItems(S30PacketWindowItems packetIn) {
        ...
        if (packetIn.getWindowId() == 0) {
            if (packetIn.getItemStacks().length == 46) {
                ItemStack[] old = packetIn.getItemStacks();
                ItemStack[] items = new ItemStack[45];
                System.arraycopy(old, 0, items, 0, 45);

                packetIn.setItemStacks(items);
            }
    ...
    public void handleCombatEvent(S42PacketCombatEvent packetIn) {
        ...
        if (packetIn.eventType == S42PacketCombatEvent.Event.END_COMBAT) {
            ...
            //TODO investigate
            if (entitylivingbase != null) {
                log.info("Combat ended between " + this.gameController.thePlayer.getName() + " and " + entitylivingbase.getName());
            } else {
                log.info("Combat ended between " + this.gameController.thePlayer.getName() + " and others");
            }
            log.info("END COMBAT - " + i);
        }
    }

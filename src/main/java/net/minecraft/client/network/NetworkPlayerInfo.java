...
public class NetworkPlayerInfo {
    ...
    public boolean hasLocationSkin() {
        if (MythProtocol.getSkin(gameProfile.getId()).isPresent()) {
            return true;
        }
    ...
    public String getSkinType() {
        Optional<MythProtocol.SkinData> skin = MythProtocol.getSkin(gameProfile.getId());

        if (skin.isPresent()) {
            return skin.get().getSkinType() == 1 ? "slim" : "default";
        }

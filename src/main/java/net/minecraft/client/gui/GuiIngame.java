...
public class GuiIngame extends Gui {
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        BlinkFix.getInstance().glslShaderUpdate();
        ...
        GlStateManager.pushMatrix();
        BlinkFix.getInstance().getEventManager().call(new EventRender2D(scaledresolution));
        GlStateManager.color(1, 1, 1, 1);
        GlStateManager.popMatrix();

        if (this.titlesTimer > 0 && !BlinkFix.getInstance().getModuleManager().getModule(NoTitle.class).isEnabled()) {
        ...
        GlStateManager.pushMatrix();
        BlinkFix.getInstance().getEventManager().call(new EventRenderUI(scaledresolution));
        GlStateManager.color(1, 1, 1, 1);
        GlStateManager.popMatrix();
    }
    ...
    @EventTarget
    public void renderShader(EventShader e) {
        if (modern) {
            RenderUtils.drawRoundedRect(position[0], position[1], position[2], position[3], 5, 0xFFFFFFFF);
        }
    }
    ...
    private void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes) {
        ...
        BaseFontRender font = modern ? siyuan16 : this.getFontRenderer();
        ...
        int i1 = collection.size() * font.getFontHeight();
        float j1 = scaledRes.getScaledHeight() / 2f + i1 / 3f;

        if (module.isEnabled()) {
            j1 += module.down.getCurrentValue();
        }

        int k1 = 3 + (modern ? 2 : 0);
        int l1 = scaledRes.getScaledWidth() - i - k1 - (modern ? 5 : 0);
        int j = 0;
        ...
        if (modern) {
            ...
            position[0] = l1 - 2;
            position[1] = j1 - collection.size() * font.getFontHeight() - font.getFontHeight() - 3;
            position[2] = l;
            position[3] = j1;

            RenderUtils.drawRoundedRect(position[0], position[1], position[2], position[3], 5, 0xFFFFFFFF);
            StencilUtils.erase(true);
            RenderUtils.drawRect(position[0], position[1], position[2], position[1] + 3, headerColor);
        }
        ...
        for (Score score1 : collection) {
            ...
            float k = j1 - j * font.getFontHeight();

            if (modern) {
                drawRect(l1 - 2, k, l, k + font.getFontHeight(), bodyColor);
            } else {
                drawRect(l1 - 2, k, l, k + font.getFontHeight(), 1342177280);
            }

            font.drawString(s1, l1, k, 0xFFFFFFFF);

            if (!module.isEnabled() || !module.noRenderPoint.getCurrentValue()) {
                font.drawString(s2, l - font.getStringWidth(s2) - (modern ? 5 : 0), k, 0xFFFFFFFF);
            }

            if (j == collection.size()) {
                String s3 = objective.getDisplayName();
                if (modern) {
                    drawRect(l1 - 2, k - font.getFontHeight() - 1, l, k - 1, bodyColor);
                    drawRect(l1 - 2, k - 1, l, k, bodyColor);
                } else {
                    drawRect(l1 - 2, k - font.getFontHeight() - 1, l, k - 1, 1610612736);
                    drawRect(l1 - 2, k - 1, l, k, 1342177280);
                }
                font.drawString(s3, l1 + i / 2f - font.getStringWidth(s3) / 2f, k - font.getFontHeight(), 0xFFFFFFFF);
            }
        }
    ...
    public BaseFontRender getFontRenderer() {

...
public abstract class GuiScreen extends Gui implements GuiYesNoCallback {
    ...
    protected boolean handleComponentClick(IChatComponent component) {
        if (component == null) {
            ...
            if (isShiftKeyDown()) {
                ...
                } else if (clickevent.getAction() == ClickEvent.Action.JOIN_TEAM) {
                    GermMod.sendPacket(GermPacketUtils.buildJoinTeamPacket(clickevent.getValue()));
                    GermMod.sendPacket(Packets.closeTeamList);
                    GermMod.sendPacket(Packets.closeTeamCreate);
                } else if (clickevent.getAction() == ClickEvent.Action.WARP_TEAMMATES) {
                    GermMod.sendPacket(Packets.warpTeammatesButtonClick);
                    GermMod.sendPacket(Packets.performWarpTeammatesAction);
                } else if (clickevent.getAction() == ClickEvent.Action.KICK_MEMBERS) {
                    if (clickevent.getValue() == null) {
                        GermMod.sendPacket(Packets.kickMembersButtonClick);
                        GermMod.sendPacket(Packets.performKickMembersAction);
                    } else {
                        GermMod.sendPacket(GermPacketUtils.buildKickMemberPacket(clickevent.getValue()));
                    }
                } else if (clickevent.getAction() == ClickEvent.Action.LEAVE_TEAM) {
                    GermMod.sendPacket(Packets.leaveTeamButtonClick);
                    GermMod.sendPacket(Packets.performLeaveTeamAction);
                } else if (clickevent.getAction() == ClickEvent.Action.DISMISS_TEAM) {
                    GermMod.sendPacket(Packets.dismissButtonClick);
                    GermMod.sendPacket(Packets.performDismissAction);
                } else {
    ...
    public void sendChatMessage(String msg) {
        EventClientChat event = new EventClientChat(msg);
    ...
    public void drawWorldBackground(boolean blur) {
        if (this.mc.theWorld != null) {
            this.drawGradientRect(0, 0, this.width, this.height, -1072689136, -804253680);
/*            StencilUtils.write(false);
            RenderUtils.drawBoundRoundedRect(0, 0, this.width, this.height, 0,new Color(0,0,0,150).getRGB());
            StencilUtils.erase(true);

            RenderUtils.drawRectBound(0, 0, this.width, this.height, new Color(0,0,0,150).getRGB());
            StencilUtils.dispose();

            if (ShaderUtils.isSupportGLSL() && blur) {
                    ...
                    DropShadowUtils.renderDropShadow(bloomFramebuffer.framebufferTexture, 10, 4, true);
                    StencilUtils.write(false);
                    RenderUtils.drawBoundRoundedRect(0, 0, this.width, this.height, 0, new Color(0, 0, 0, 150).getRGB());
                    StencilUtils.erase(true);

...
public class BlockModelRenderer {
    ...
    public boolean renderModel(IBlockAccess blockAccessIn, IBakedModel modelIn, IBlockState blockStateIn, BlockPos blockPosIn, WorldRenderer worldRendererIn, boolean checkSides) {
        Block block = blockStateIn.getBlock();
        BlinkFix.getInstance().getEventManager().call(new EventRenderBlock(block, blockPosIn));
        boolean flag = (Minecraft.isAmbientOcclusionEnabled() || (XRay.isXrayEnabled())) && block.getLightValue() == 0 && modelIn.isAmbientOcclusion();
    ...
    private boolean renderModelSmooth(IBlockAccess p_renderModelSmooth_1_, IBakedModel p_renderModelSmooth_2_, IBlockState p_renderModelSmooth_3_, BlockPos p_renderModelSmooth_4_, WorldRenderer p_renderModelSmooth_5_, boolean p_renderModelSmooth_6_) {
        ...
        XRay module = (XRay) BlinkFix.getInstance().getModuleManager().getModule(XRay.class);

        boolean isOre = module.isValidBlock(block);
        ...
        boolean isXrayOn = XRay.isXrayEnabled();
        boolean forceRender = isXrayOn && isOre;
        ...
        for (EnumFacing enumfacing : EnumFacing.VALUES) {
            ...
            if (!list.isEmpty()) {
                ...
                boolean originFlag = !p_renderModelSmooth_6_ || block.shouldSideBeRendered(p_renderModelSmooth_1_, blockpos, enumfacing);
                if (originFlag || forceRender) {

...
public class ItemRenderer {
    ...
    private void doBlockTransformations() {
        ...
        float[] rotation = BlockAnimation.getRotation();

        if (rotation != null) {
            GlStateManager.rotate((rotation[0] + 1) * 30F, 0, 1, 0);
            GlStateManager.rotate((rotation[1] + 1) * -80F, 1, 0, 0);
            GlStateManager.rotate((rotation[2] + 1) * 60F, 0, 1, 0);
        } else {
        ...
        if (!Config.isShaders() || !Shaders.isSkipRenderHand()) {
            ...
            if (this.itemToRender != null) {
                if (this.itemToRender.getItem() instanceof ItemMap) {
                    ...
                    boolean auraBlocked = Aura.blocked && itemToRender.getItem() instanceof ItemSword;
                    boolean renderBlock = Aura.renderBlock && itemToRender.getItem() instanceof ItemSword;
                    if (abstractclientplayer.getItemInUseCount() > 0 || auraBlocked || renderBlock) {
                        EnumAction enumaction = auraBlocked ? EnumAction.BLOCK : this.itemToRender.getItemUseAction();
                        ...
                        switch (enumaction) {
                            ...
                            case BLOCK:
                                String blockAnimation = BlockAnimation.getBlockAnimation();
                                if (blockAnimation.equals("Vanilla")) {
                                    this.transformFirstPersonItem(f, 0.0F);
                                    this.doBlockTransformations();
                                } else {
                                    this.renderBlockingAnimation(blockAnimation, f1, f);
                                }
    ...
    // Client
    public void renderBlockingAnimation(String mode, float swingProgress, float equippedProgress) {
        equippedProgress *= BlockAnimation.getEquippedProgress();
        swingProgress = MathUtils.clampValue(swingProgress / BlockAnimation.getSwingProgress(), 0, 1);

        float[] position = BlockAnimation.getPosition();
        ...
        switch (mode) {
            ...
            case "Push":
                ...
                GlStateManager.rotate(f * -10F, 0.0F, 1.0F, 0.0F);
                GlStateManager.rotate(f1 * -10F, 0.0F, 0.0F, 1.0F);
    ...
    ItemStack lastRenderItemStack;

    public void updateEquippedItem() {
        this.prevEquippedProgress = this.equippedProgress;
        EntityPlayer entityplayer = this.mc.thePlayer;

        if (mc.entityRenderer.skipRender > 0) {
            mc.entityRenderer.skipRender--;
        } else {
            lastRenderItemStack = entityplayer.inventory.getCurrentItem();
        }

        boolean flag = false;

        if (this.itemToRender != null && lastRenderItemStack != null) {
            if (!this.itemToRender.getIsItemStackEqual(lastRenderItemStack)) {
                flag = true;
            }
        } else flag = this.itemToRender != null || lastRenderItemStack != null;
        ...
        if (this.equippedProgress < 0.1F) {
            this.itemToRender = lastRenderItemStack;
            this.equippedItemSlot = entityplayer.inventory.currentItem;

            if (Config.isShaders()) {
                Shaders.setItemToRenderMain(lastRenderItemStack);
            }
        }
    }

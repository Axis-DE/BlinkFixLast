...
public class InventoryManager extends Module {
    ...
    FloatValue delay = ValueBuilder.create(this, "Delay").setDefaultFloatValue(100).setMinFloatValue(0).setMaxFloatValue(1000).setFloatStep(20).build().getFloatValue();
    FloatValue opendelay = ValueBuilder.create(this, "Open Delay").setDefaultFloatValue(100).setMinFloatValue(0).setMaxFloatValue(300).setFloatStep(20).build().getFloatValue();
    BooleanValue openinv = ValueBuilder.create(this, "Open Inv").setDefaultBooleanValue(false).build().getBooleanValue();
    ...
    BooleanValue openInvDelay = ValueBuilder.create(this, "Open Inv Delay").setDefaultBooleanValue(false).setVisibility(() -> openinv.getCurrentValue()).build().getBooleanValue();
    ...
    private boolean waitingForOpenInvDelay = false;
    private final TimeHelper openInvDelayTimer = new TimeHelper();
    ...
    public void onPreUpdate(EventMotion event) {
        if (event.getType() == EventType.PRE) {
            ...
            if (BlinkFix.getInstance().getModuleManager().getModule(AutoGapple.class).isEnabled()) return;

            if (mc.thePlayer.isUsingItem()) {
                return;
            }

            if (openinv.getCurrentValue() && openInvDelay.getCurrentValue()) {

                if (mc.currentScreen instanceof GuiInventory) {
                    if (waitingForOpenInvDelay) {

                        if (openInvDelayTimer.delay(opendelay.getCurrentValue())) {
                            waitingForOpenInvDelay = false;
                        } else {
                            return;
                        }
                        ...
                        waitingForOpenInvDelay = true;
                        openInvDelayTimer.reset();
                        return;
                    }
                } else {
            ...
            if (openinv.getCurrentValue() && !(mc.currentScreen instanceof GuiInventory)) {
                return;
            }
            ...
            if (mc.currentScreen == null || mc.currentScreen instanceof GuiInventory || mc.currentScreen instanceof GuiChat) {
                ...
                for (int slotIndex = 9; slotIndex < 45; slotIndex++) {
                    ItemStack stack = mc.thePlayer.getSlotFromPlayerContainer(slotIndex).getStack();
                    if (stack != null) {
                        if (!BlinkFix.getInstance().getModuleManager().getModule(Blink.class).isEnabled()) {

...
//    ModeValue targetespmode = ValueBuilder.create(this, "Naven").setModes("Naven", "", "", "").setVisibility(() -> !smart.getCurrentValue()).build().getModeValue();
...
//            autoBlock.setCurrentValue(true);
    ...
    public boolean onEnable() {
        if (this.range.getCurrentValue() > 3.0 && ServerUtils.serverType == ServerUtils.ServerType.GERM_PLUGIN) {
            BlinkFix.getInstance().getNotificationManager().addNotification(new Notification(NotificationLevel.WARNING, "You are using a high range, may result ban!", 3000));
        }
        ...
        if (event.getType() == EventType.PRE) {
            ...
            if (blocked && autoBlock.isCurrentMode("GrimAC")) {
                if (ViaLoadingBase.getInstance().getTargetVersion().isNewerThan(ProtocolVersion.v1_9)) {
                    if (BlinkFix.getInstance().getModuleManager().getModule(Blink.class).isEnabled()) return;
...
//    boolean isSwapped;\
    ...
    public boolean shouldBlock() {
        ...
        return isEnabled() && hasTarget && hasSword() && !BlinkFix.getInstance().getModuleManager().getModule(OffhandFeatures.class).isEnabled();
    }
            ...
            for (EntityLivingBase target : targets) {
                ...
                if (distance < currentRange) {
                    if (breaker.getCurrentValue() && mc.thePlayer.getHealth() > breakhealth.getCurrentValue()) {
                        working = false;
                        int axeSlot = findAxe();
                        if (axeSlot != -100 && !BlinkFix.getInstance().getModuleManager().getModule(AutoGapple.class).isEnabled()) {
                        ...
                        working = true;
                        if (Critical.shouldCritical()) {
                        }
                    }
                    attackEntity(target);
                    BlinkFix.getInstance().getEventManager().call(new EventAttack(false));
                }
            }
        }
    }

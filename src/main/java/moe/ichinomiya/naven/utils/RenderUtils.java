...
public class RenderUtils {
    ...
    public static float elasticOut(float t) {
        return (float) (Math.sin(-13.0 * (t + 1) * Math.PI / 2) * Math.pow(2.0, -10.0 * t) + 1.0);
    }

    public static float elasticInOut(float t) {
        if (t < 0.5) {
            return (float) (0.5 * Math.sin(13.0 * Math.PI / 2 * 2.0 * t) * Math.pow(2.0, 10.0 * (2.0 * t - 1.0)));
        } else {
            return (float) (0.5 * (Math.sin(-13.0 * Math.PI / 2 * ((2.0 * t - 1.0) + 1.0)) * Math.pow(2.0, -10.0 * (2.0 * t - 1.0)) + 2.0));
        }
    }
    public static void drawRoundedRect(float x, float y, float width, float height, float radius, Color color) {
        drawRoundedRect(x, y, x + width, y + height, radius, color.getRGB());
    }
    public static void drawRoundedRectWithDifferentCorners(float x, float y, float width, float height,
                                                           float topLeft, float topRight,
                                                           float bottomLeft, float bottomRight, Color color) {
        if (topLeft <= 0 && topRight <= 0 && bottomLeft <= 0 && bottomRight <= 0) {
            drawRect(x, y, x + width, y + height, color.getRGB());
            return;
        }
        if (topLeft == topRight && topLeft == bottomLeft && topLeft == bottomRight) {
            drawRoundedRect(x, y, width, height, topLeft, color);
            return;
        }
        ...
        GL11.glDisable(GL11.GL_TEXTURE_2D);
        GL11.glBlendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
        drawRect(x + topLeft, y, right - topRight, bottom, color.getRGB());
        drawRect(x, y + topLeft, x + topLeft, bottom - bottomLeft, color.getRGB());
        drawRect(right - topRight, y + topRight, right, bottom - bottomRight, color.getRGB());
        if (topLeft > 0) {
            drawPartialCircle(x + topLeft, y + topLeft, topLeft, 180, 270, 20, color.getRGB());
        }
        if (topRight > 0) {
            drawPartialCircle(right - topRight, y + topRight, topRight, 270, 360, 20, color.getRGB());
        }
        if (bottomLeft > 0) {
            drawPartialCircle(x + bottomLeft, bottom - bottomLeft, bottomLeft, 90, 180, 20, color.getRGB());
        }
        if (bottomRight > 0) {
            drawPartialCircle(right - bottomRight, bottom - bottomRight, bottomRight, 0, 90, 20, color.getRGB());
        }

        GL11.glEnable(GL11.GL_TEXTURE_2D);
        GL11.glDisable(GL11.GL_BLEND);
    }
    public static void drawPartialCircle(float x, float y, float radius, int startAngle, int endAngle, int segments, int color) {
        GL11.glEnable(GL11.GL_BLEND);
        GL11.glDisable(GL11.GL_TEXTURE_2D);
        GL11.glBlendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
    ...
    public static void drawRoundedGradientRect(float x, float y, float width, float height, float radius, Color color1, Color color2, boolean horizontal) {
        if (horizontal) {
            drawGradientRect(x, y, x + width, y + height, false, color1.getRGB(), color2.getRGB());
        } else {
            drawGradientRect(x, y, x + width, y + height, true, color1.getRGB(), color2.getRGB());
        }
    }
    public static void drawFrostedGlassRect(float x, float y, float width, float height, int baseColor, float blurAlpha) {
        drawRect(x, y, x + width, y + height, baseColor);

        // 第二层：亮色高光模拟玻璃反光
        drawRect(x, y, x + width, y + 1, reAlpha(0xFFFFFFFF, 0.3f));
        drawRect(x, y, x + 1, y + height, reAlpha(0xFFFFFFFF, 0.2f));

        // 第三层：白色半透明覆盖层模拟毛玻璃效果
        drawRect(x + 1, y + 1, x + width - 1, y + height - 1, reAlpha(0xFFFFFF, 0.1f));

        // 第四层：暗色阴影模拟玻璃厚度
        drawRect(x + width - 1, y, x + width, y + height, reAlpha(0x000000, 0.3f));
        drawRect(x, y + height - 1, x + width, y + height, reAlpha(0x000000, 0.3f));
    }
    // 在 RenderUtils 类中添加这个方法
    public static void drawContinuousRoundedBackground(float x, float y, float width, float height,
                                                       float cornerRadius, int color, boolean isFirst, boolean isLast) {
        if (height <= 0) return;

        if (isFirst && isLast) {
            drawRoundedRect(x, y, x + width, y + height, cornerRadius, color);
        } else if (isFirst) {
            drawRoundedRect(x, y, x + width, y + height + cornerRadius, cornerRadius, color);
            drawRect(x, y + height, x + width, y + height + cornerRadius, color);
        } else if (isLast) {
            drawRect(x, y - cornerRadius, x + width, y, color); // 覆盖顶部
            drawRoundedRect(x, y - cornerRadius, x + width, y + height + cornerRadius, cornerRadius, color);
        } else {
            drawRect(x, y - cornerRadius, x + width, y + height, color);
        }
    }
